...
p$add_argument('--atlas_umap', type="character",help="atlas umap coords")
...
meta_atlas <- fread(args$atlas_metadata) %>%
  ...
  .[pass_rnaQC==T & doublet_call==F & pass_atacQC==T]

# Extract precomputed dimensionality reduction coordinates
umap.dt <- fread(args$atlas_umap) %>% 
  .[,c("UMAP1","UMAP2","cell")] %>% 
  .[cell%in%meta_atlas$cell] %>%
  setnames(c("UMAP1","UMAP2"),c("V1","V2"))
#umap.dt <- meta_atlas %>%
