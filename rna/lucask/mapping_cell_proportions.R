here::i_am("rna/lucask/mapping_cell_proportions.R")
...
############################
## plot mapping histogram ##
############################

closest_cell_count <- sample_metadata %>% group_by(sample) %>% count(closest.cell) %>% arrange(desc(n))

p <- ggplot(closest_cell_count, aes(x = n)) + geom_histogram(binwidth = 1) + facet_wrap(~sample)
pdf(file.path(args$outdir,"mapped_cell_distribution.pdf"), width=7, height=5)
print(p)
dev.off()

for(i in args$samples){
  fwrite(closest_cell_count[closest_cell_count$sample == i,],file = file.path(args$outdir,sprintf("per_sample/%s_mapped_cell_distribution.txt.gz",i)))
}
...
pdf(sprintf("%scelltype_proportions_stacked_barplots.pdf",args$outdir), width=7, height=5)
print(p)
dev.off()
