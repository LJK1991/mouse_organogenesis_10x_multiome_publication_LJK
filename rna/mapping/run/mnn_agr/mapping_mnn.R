...
genes.intersect <- genes.intersect[!genes.intersect%in%gene_metadata[chromosome_name=="chrY",external_gene_name]]
...
#LJK-230411-modify
#changed celltype.mapped to celltype hopefully fixes alot of things
#celltype.score to mapping_mnn
mapping.dt <- mapping$mapping %>% 
  .[,c("cell","celltype.mapped","celltype.score","closest.cell")] %>% 
  as.data.table
colnames(mapping.dt) <- c('cell','celltype','mapping_mnn','closest.cell')
